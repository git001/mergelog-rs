...
impl PartialEq for Entry {
    fn eq(&self, o: &Self) -> bool {
        self.ts == o.ts && self.file_idx == o.file_idx
    }
}
impl Eq for Entry {}
...
fn main() {
    ...
    if args.is_empty() {
        eprintln!("usage: profile_timing <file>...");
        return;
    }
    ...
    eprintln!(
        "  I/O   (read_line)    {:>8.3}s  ({:4.1}%)",
        t_io.as_secs_f64(),
        t_io.as_secs_f64() / total.as_secs_f64() * 100.0
    );
    eprintln!(
        "  Parse (hand-rolled)  {:>8.3}s  ({:4.1}%)",
        t_parse.as_secs_f64(),
        t_parse.as_secs_f64() / total.as_secs_f64() * 100.0
    );
    eprintln!(
        "  Heap  (push/pop)     {:>8.3}s  ({:4.1}%)",
        t_heap.as_secs_f64(),
        t_heap.as_secs_f64() / total.as_secs_f64() * 100.0
    );
    eprintln!(
        "  Write (write_all)    {:>8.3}s  ({:4.1}%)",
        t_write.as_secs_f64(),
