...
pub fn parse_clf_timestamp(line: &str) -> Option<i64> {
    ...
    let unix =
        days_since_epoch(year, month, day) * 86_400 + hour * 3600 + min * 60 + sec - tz_offset;
...
pub(crate) fn days_to_ymd(z: i64) -> (i64, i64, i64) {
    let z = z + 719_468;
...
mod tests {
    ...
    fn parses_combined_log_line() {
        let line =
            r#"213.133.113.84 - - [06/Mar/2026:00:01:56 +0100] "GET / HTTP/1.1" 200 78 "-" "Bot""#;
    ...
    #[test]
    fn ts_to_string_epoch() {
        assert_eq!(ts_to_string(0), "1970-01-01 00:00:00 UTC");
    }

    #[test]
    fn ts_to_string_known_date() {
        let ts = days_since_epoch(2026, 3, 22) * 86_400 + 14 * 3600 + 50 * 60 + 51;
        assert_eq!(ts_to_string(ts), "2026-03-22 14:50:51 UTC");
    }

    #[test]
    fn ts_to_string_sentinel_values() {
        assert_eq!(ts_to_string(i64::MAX), "N/A");
        assert_eq!(ts_to_string(i64::MIN), "N/A");
    }

    #[test]
    fn sentinel_is_max() {
        assert_eq!(sentinel(), i64::MAX);
    }
}
